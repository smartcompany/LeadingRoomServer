...
export function analyzeTechnical(bars1d) {
    const source = bars1d;
    ...
    const rsiRecoveringFromOversold = rsiValue !== null &&
        prevRsi !== null &&
        prevRsi < 30 &&
        rsiValue >= 30 &&
        rsiValue < 50;
    if (rsiRecoveringFromOversold)
        notes.push('RSI 과매도 회복');
    const { hist: macdHist, bullishCross: macdBullishCross } = macdHistogram(closes);
    if (macdBullishCross)
        notes.push('MACD 상향 교차');
    ...
    if (atr !== null)
        notes.push(`ATR 손절 추정 ${atr.toFixed(1)}%`);
    ...
    if (volumeRatio !== null && volumeRatio >= 2.4)
        score += 0.15;
    if (nearResistanceBreak && trend === 'up')
        score += 0.1;
