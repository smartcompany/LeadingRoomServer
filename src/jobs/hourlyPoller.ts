...
import type { CandleBar, MarketId, SymbolRow, Timeframe } from '../types/index.js';
...
async function processSymbol(symbol: SymbolRow): Promise<void> {
...
export async function runHourlyPoll(options?: { marketIds?: MarketId[] }): Promise<void> {
  const client = getAdminClient();
  let query = client
    .from('lr_symbols')
    .select('*')
